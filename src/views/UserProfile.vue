<template>
  <div>
    <v-row align="center" justify="center" class="pt-10">
      <v-progress-circular size="60" color="#FFD831" v-model="loading"  indeterminate></v-progress-circular>
    </v-row>
      ...
      <v-row>
        ...
        <v-col class="pl-10" cols="3" align-self="center">
          ...
          <p class="text--secondary">
            a web developer in aswar academy
            <br />in Development department
          </p>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="3">
          ...
          <div class="pt-4">
            ...
            <div>
              <v-icon class="mr-1">mdi-av-timer</v-icon>
              <strong>3 Years Job</strong>

              <p class="pl-7 text--secondary">experience</p>
            </div>
            <div>
              <v-icon class="mr-1">mdi-calendar-range</v-icon>
              <strong>The date of join</strong>

              <p class="pl-7 text--secondary">{{ user.createdAt }}</p>
            </div>
            <div>
              <v-container id="user-div2" class="pa-0 ma-0">
                <strong style="color: #D82D37;">The number of absences</strong>
              </v-container>
              <p class="text--secondary pl-7 pt-2">20 in this month</p>
            </div>
          </div>
        </v-col>
...
<script lang="ts">
import Vue from "vue";
import { UserDto, UsersService, UserData } from "@/client";
...
export default Vue.extend({
  ...
  methods: {
    getUser() {
      this.loading = true;
      UsersService.usersControllerFindOne(3).then(value => {
        this.user = value;
        this.id = value.id;
      });
      this.loading = false;
